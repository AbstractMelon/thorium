...
class ShuttleBay extends Component {
  ...
  render() {
    ...
    if (direction === "departing" && docked) {
      ...
      if (clamps) hint.clamps = "attention";else
      if (compress) hint.compress = "attention";else
      if (doors) hint.doors = "attention";else
      shuttleStatusMsg = "Shuttle is departing.";
      ...
      if (!doors) hint.doors = "attention";else
      if (!compress) hint.compress = "attention";else
      shuttleStatusMsg = "Shuttle bay secured.";
      ...
      if (compress) hint.compress = "attention";else
      if (doors) hint.doors = "attention";else
      shuttleStatusMsg = "The shuttle is entering the bay. Please wait.";
    } else if (direction === "arriving" && docked) {
      ...
      if (!compress) hint.compress = "attention";else
      if (!clamps) hint.clamps = "attention";else
      shuttleStatusMsg = "Shuttle secured.";
    } else if (!docked) {
      if (!clamps) hint.clamps = "ok";
      if (compress) hint.compress = "ok";
      if (doors) hint.doors = "ok";
